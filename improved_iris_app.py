...
st.title('iris speices prediction app')
s_len=st.sidebar.slider('SepalLength',float(iris_df['SepalLengthCm'].min()),float(iris_df['SepalLengthCm'].max()))
s_wid=st.sidebar.slider('SepalWidth',float(iris_df['SepalWidthCm'].min()),float(iris_df['SepalWidthCm'].max()))
p_len=st.sidebar.slider('PetalLength',float(iris_df['PetalLengthCm'].min()),float(iris_df['PetalLengthCm'].max()))
p_wid=st.sidebar.slider('PetalWidth',float(iris_df['PetalWidthCm'].min()),float(iris_df['PetalWidthCm'].max()))
...
if st.sidebar.button('predict'):
  if classifier=='support vector machine':
    ...
    species=prediction(log_reg,s_len,s_wid,p_len,p_wid)
    score=log_reg.score(X_train,y_train)
  else :
    species=prediction(rf_clf,s_len,s_wid,p_len,p_wid)
    score=rf_clf.score(X_train,y_train)
  st.write(f'Species indentified => {species} Accuracy of model => {score}')